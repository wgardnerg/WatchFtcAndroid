package com.wrgardnersoft.watchftctournament.internet;
    ...
    protected Void doInBackground(Void... params) {
        ...
        for (int i = 0; (i < urlSuffix.length) && (!serverOK); i++) {
            ...
            try {
                Document document;
                try {
                    teamsUrl = teamsUrlHeader + "TeamList" + suffix[0];
                    document = Jsoup.connect(teamsUrl).get();
                } catch (Exception e1) {
                    teamsUrl = teamsUrlHeader + "TeamList" + suffix[1];
                    document = Jsoup.connect(teamsUrl).get();
                }
                ...
                try {
                    teamsUrl = teamsUrlHeader + "Rankings" + suffix[0];
                    document = Jsoup.connect(teamsUrl).get();
                } catch (Exception e1) {
                    teamsUrl = teamsUrlHeader + "Rankings" + suffix[1];
                    document = Jsoup.connect(teamsUrl).get();
                }
                ...
                try {
                    teamsUrl = teamsUrlHeader + "MatchDetails" + suffix[0];
                    document = Jsoup.connect(teamsUrl).get();
                } catch (Exception e1) {
                    teamsUrl = teamsUrlHeader + "MatchDetails" + suffix[1];
                    document = Jsoup.connect(teamsUrl).get();
                }
