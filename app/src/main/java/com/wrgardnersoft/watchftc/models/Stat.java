...
public class Stat {
    ...
    //        int meanOffenseMatchCount[] = new int[numTeams];
            double meanOffenseScoreTotal[] = new double[MyApp.ScoreType.values().length];
            ...
            if (numMatches > 0) {
                ...
                /*        meanOffenseMatchCount[atn.indexOf(m.teamNumber[MyApp.RED][0])]++;
                        meanOffenseMatchCount[atn.indexOf(m.teamNumber[MyApp.RED][1])]++;
                        meanOffenseMatchCount[atn.indexOf(m.teamNumber[MyApp.BLUE][0])]++;
                        meanOffenseMatchCount[atn.indexOf(m.teamNumber[MyApp.BLUE][1])]++;*/
                    }
                }
                ...
                for (MyApp.ScoreType type : MyApp.ScoreType.values()) {
                    ...
                    for (int i = 0; i < myApp.match[myApp.division()].size(); i++) {
                        ...
                        if ((m.score[MyApp.RED][MyApp.ScoreType.TOTAL.ordinal()] >= 0) &&
                            ...
                            meanOffenseScoreTotal[type.ordinal()] += BoprA.get(iM, 0);
                            iM++;
                    ...
                    for (int i = 0; i < 2 * numMatches; i++) {
                        BoprA.set(i, 0, BoprA.get(i, 0) - 2 * meanOffenseScoreTotal[type.ordinal()]);
                    }
                    ...
                    for (int i = 0; i < numTeams; i++) {
                        myApp.teamStatRanked[division].get(i).oprA[type.ordinal()] += meanDpr + meanOffenseScoreTotal[type.ordinal()];
